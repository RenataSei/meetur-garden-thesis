...
router.get("/", async (req, res) => {
  try {
    ...
    if (city) {
      ...
      return res
        .status(400)
        .json({ error: "City or lat/lon query parameters are required" });
    }
    ...
    if (error.response) {
      ...
      res
        .status(500)
        .json({ error: "Network error, no response from weather API" });
    } else {
...
router.get("/forecast", async (req, res) => {
  ...
  // ---------------------------------------------------------
  // 🟢 NEW: MAP CONFIG ROUTE
  // ---------------------------------------------------------
  // @route   GET /api/weather/config
  // @desc    Securely hand the API key to the frontend map
  // @access  Public
  router.get("/config", (req, res) => {
    ...
    if (!apiKey) {
      return res
        .status(500)
        .json({ error: "Weather API key not configured on server" });
    }
